...
| Date | PR | Model | Gate | Reviewer said | Human did | Direction | Reason |
|---|---|---|---|---|---|---|---|
| 2026-09-22 | #522 | publishers/google/models/gemini-3.1-pro-preview | framing | framing fail: The PR claims the new tests are integrated into `npm test`, but they are only added as a standalone script. | **merged over** | PENDING-HUMAN | PENDING-HUMAN — edit this row, then approve |
...
| 2026-08-28 | #465 | publishers/google/models/gemini-3.1-pro-preview | premise | premise fail: Unnecessary and undisclosed addition of a Node engine restriction. | **merged over** | reviewer misinterpreted | The reviewer treated engines.node: ">=24" in tools/executive-assistant/package-lock.json as a new, undisclosed restriction. It is not. package.json on develop already has that field. It was added in 69255be (chore: align executive-assistant with Node 24) without regenerating the lockfile. Dependabot’s npm install for the vitest 4.1.10 → 4.1.11 bump copied the existing package.json engines block into lockfile v3 metadata. That is normal npm behavior, not a new policy. Removing engines from the lockfile would restore the drift from 69255be and would not change runtime policy. Fleet baseline is already Node 24. |
