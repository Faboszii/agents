...
Per the Product Owner directive of 2026-08-03 (Decision [2026-08-03-0002], Requirement §7; canonical runbook `docs/MACHINE_IDENTITY.md`), agent-initiated PRs MUST be authored by the `noemi-agent` machine identity — never with a human's credentials, which would make the PR unreviewable by that human (GitHub blocks author self-approval):
...
| `DRAFT_HELD` | Stop, whatever the checks say. The PR is a **draft the machine identity did not author**, so the gate declined to act on it *at all* — it will not reopen it, will not order commits onto it, will not file an issue against it, and will not take it out of draft. Draft is the author's not-ready signal and it outranks you. Report the verdict and name the author who must mark it ready. Do **not** mark it ready yourself even if your credentials allow it: that is exactly the automation-overrides-a-human move this verdict exists to prevent. If your own PRs keep landing here, the cause is the `AGENT_GH_TOKEN` injection gap (Clarification [2026-08-09]) — your PR is authored by a human account instead of `noemi-agent`; injecting the token makes your drafts bot-authored and this verdict stops occurring. |
